...
export const UsewpgTableStore = defineStore('dailywpgtable', () => {
    ...
    const filtertableData = computed(() => {
        return tableData.value/////時間區間產出的批
            .filter((item) => item.ProdClass === deviceRecord.value
                && item.PartNum === partRecord.value
                &&item.Yield-item.Triger<0)
    });////產出且被Tri的批

    const filterTotaltableData=computed(()=>{
        return tableData.value.filter((item)=>item.ProdClass === deviceRecord.value&&item.PartNum === partRecord.value);
    });////個別產出
    ...
    const pntritarCount=computed(()=>{///預設會先跑一次
        ...
        pnAry.value.forEach((eachpart)=>{
            countBox.push({
                pn:eachpart,
                count:tableData.value.filter((item)=>item.PartNum===eachpart).length,
                green:tableData.value.filter((item)=>item.PartNum===eachpart&&Number(item.Target)-Number(item.Yield)<=0).length,
                yellow:tableData.value.filter((item)=>item.PartNum===eachpart&&Number(item.Triger)-Number(item.Yield)<=0&&Number(item.Target)-Number(item.Yield)>0).length,
                red:tableData.value.filter((item)=>item.PartNum===eachpart&&Number(item.Triger)-Number(item.Yield)>0).length
            })
        })
        return countBox
    });
