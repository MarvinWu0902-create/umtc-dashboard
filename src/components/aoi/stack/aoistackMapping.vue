...
const { partData, param, stackData } = storeToRefs(useaoistackstore);
    ...
    if (props.Week&&props.defect&&stackData.value.length>0) {///條件進入或是要watchEffect的值
        ...
        nextTick(() => {
            ...
            const mappingdata= stackData.value
                ...
                .map((item)=>
                [
                    item.Unit_X.charCodeAt(0) - 'A'.charCodeAt(0) + 1,
                    Number(item.Unit_Y),
                    Number(props.defect==='Yield'?item['All']:item[`${props.defect}`])
                    
                ]);

            const xAxisAry=[...new Set(stackData.value.map((item)=>item.Unit_X.charCodeAt()-65))]

            const yAxisAry=[...new Set(stackData.value.map((item)=>Number(item.Unit_Y)-1))].sort((a,b)=>a-b)
