...
const { partData, param, yieldData, weekData,navpartRecord,radioItem } = storeToRefs(useaoistackstore);
        ...
        nextTick(() => {
            ...
            weekData.value.forEach((week, weekIndex) => {
                ...
                const scatter_data = scatter_obj.map((item) =>
                    [
                        weekIndex,
                        props.defect === 'Yield' ? Number(item['Bef_Yield']) : Number(item[`${props.defect}`]),
                        item.LotNum,
                        item.LayerName,
                        item.LotType,
                        item.Bef_Yield,
            ...
            Highcharts.chart(chart.value, {
                ...
                title: {
                    text: `${radioItem.value} ${navpartRecord.value} ${props.defect}`
              ...
              Low: ${(boxplotValues.low * 100).toFixed(2)}%<br>`;
                    ...
                    },
                    {
                        ...
                        <p>${this.series.userOptions.data[this.index][4]}</p>`;
