...
const { partData, param, stackData } = storeToRefs(usebumpstackstore);
...
watchEffect(() => {
    ...
    if (props.Week&&props.defect&&stackData.value.length>0) {///條件進入或是要watchEffect的值
        ...
        nextTick(() => {
            console.log('2');
            console.log('stackData.value',stackData.value);
            ...
            const xAxisAry=[...new Set(stackData.value.map((item)=>Number(item.Unit_X)-1))]
                .sort((a,b)=>a-b)
                .unshift(0)

            const yAxisAry=[...new Set(stackData.value.map((item)=>Number(item.Unit_Y)-1))]
                .sort((a,b)=>a-b)
                .unshift(0)
