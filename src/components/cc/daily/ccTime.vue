...
const {cc_params}=storeToRefs(useccstore);
...
cc_params.value.st = range.value[0];///aoi_params 從fetch 傳進=>預設綁定
cc_params.value.et = range.value[1];

const clickHandler = () => {
    cc_params.value.st = range.value[0];
    cc_params.value.et = range.value[1];
    ccfetchData();
};
