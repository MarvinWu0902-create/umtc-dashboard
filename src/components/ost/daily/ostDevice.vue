<template>
    <div class="layout">
        <button 
        :class="{ active: deviceRecord === eachDevice.ProdClass }" 
        @click="btnClick(eachDevice.ProdClass)"
        v-for="eachDevice of deviceData"
        >
        {{ eachDevice.ProdClass }}({{ deviceCount[eachDevice.ProdClass] }})
    </button>
    </div>
</template>
...
const {tableData,deviceData,deviceCount}=storeToRefs(useoststore);
